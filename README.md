...
- [x] Структура данных для ServiceConfig
- [x] Парсинг JSON-конфига
- [x] Валидация конфигурации (обязательные поля, типы)
- [x] Загрузка конфига при старте приложения
- [ ] Горячая перезагрузка конфига (опционально)

### 🌐 Реализация проверок
- [x] Базовый интерфейс `AbstractChecker`
- [x] Реализация TCP-проверки (Boost.Asio)
- [x] Реализация ICMP Ping (Boost.Asio)
- [ ] Реализация HTTP-проверки

### ⏱ Тайминги и статистика
- [ ] Периодический опрос (`asio::steady_timer`)
- [x] Сбор времени отклика (Boost(STL).Chrono)
- [ ] Базовая статистика (last/min/max/avg)

### 🧾 Логирование
- [x] Singleton Logger с потокобезопасностью
- [x] Файловое и консольное логирование через Boost.Log
- [ ] Логи в UI
